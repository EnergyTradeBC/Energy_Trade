...
func (s *SmartContract) Mint(ctx contractapi.TransactionContextInterface, recipient string, amount int) error {
	...
	err = ctx.GetStub().PutState(minter, []byte(strconv.Itoa(0)))
	if err != nil {
		return err
	}

	err = transferHelperMint(ctx, minter, recipient, amount)
	if err != nil {
...
func (s *SmartContract) Transfer(ctx contractapi.TransactionContextInterface, recipient string, amount int) error {
	...
	initialized, err := checkInitialized(ctx)
	if err != nil {
		return err //fmt.Errorf("failed to check if contract is already initialized: %v", err)
	}
	if !initialized {
		return err //fmt.Errorf("Contract options need to be set before calling any function, call Initialize() to initialize contract")
	}

	// Get ID of submitting client identity
	clientID, err := ctx.GetClientIdentity().GetID()
	if err != nil {
		return err //fmt.Errorf("failed to get client id: %v", err)
	}

	err = transferHelper(ctx, clientID, recipient, amount)
	if err != nil {
		return err //fmt.Errorf("failed to transfer: %v", err)
	}
...
func transferHelperMint(ctx contractapi.TransactionContextInterface, from string, to string, value int) error {
