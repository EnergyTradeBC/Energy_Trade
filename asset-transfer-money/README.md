# token scenario
In this scenario, which is an account-based model, there is an account for each participant that holds a balance of tokens. The tokens are fungible, meaning that there are interchangeable and sumable with each other.
A mint transaction creates tokens in the minter account, while a transfer transaction debits the caller's account and credits another account.
...
Accounts are defined at the client identity level, where every authorized client with an enrollment certificate from their organization implicitly has an account ID that matches their client ID.
