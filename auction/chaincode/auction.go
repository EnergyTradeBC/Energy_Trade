...
func (s *SmartContract) CreateAuction(ctx contractapi.TransactionContextInterface, auctionID string, itemsold string, quantity float32, startingBid float32, withAuditor string) error {
	...
	err = ctx.GetStub().SetEvent("createAuction", createAuctionEventJSON)
	if err != nil {
...
func (s *SmartContract) RevealBid(ctx contractapi.TransactionContextInterface, auctionID string, txID string) error {
	...
	// check that the bidInput has a price higher than the StartingBid

	if bidInput.Price < auction.StartingBid {
		return fmt.Errorf("bid amount should be greater than the starting bid")
	}
...
func (s *SmartContract) CloseAuction(ctx contractapi.TransactionContextInterface, auctionID string) error {
	...
	err = ctx.GetStub().SetEvent("closeAuction", closeAuctionEventJSON)
	if err != nil {
		return fmt.Errorf("failed to set event: %v", err)
	}
...
func (s *SmartContract) EndAuction(ctx contractapi.TransactionContextInterface, auctionID string) error {
	...
	err = ctx.GetStub().SetEvent("endAuction", endAuctionEventJSON)
	if err != nil {
		return fmt.Errorf("failed to set event: %v", err)
	}
